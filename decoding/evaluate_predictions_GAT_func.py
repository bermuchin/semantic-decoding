...
if __name__ == "__main__":
    ...
    # [수정됨] GAT 결과 파일 로드 (_GAT_func 접미사)
    pred_path = os.path.join(config.RESULT_DIR, args.subject, args.experiment, f"{args.task}_GAT_func.npz")
    print(f"[*] Loading func GAT results from: {pred_path}")
    ...
    # [수정됨] 결과 저장 (_GAT_func 접미사 적용)
    save_location = os.path.join(config.SCORE_DIR, args.subject, args.experiment)
    os.makedirs(save_location, exist_ok=True)
    save_path = os.path.join(save_location, f"{args.task}_GAT_func.npz")
