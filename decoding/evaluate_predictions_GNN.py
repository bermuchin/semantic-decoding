...
if __name__ == "__main__":
    ...
    parser.add_argument("--null", type = int, default = 10) # 통계적 유의성 검증을 위해 기본값 10 유지 권장
    ...
    # [MODIFICATION] Load GNN prediction transcript (_GNN suffix)
    pred_path = os.path.join(config.RESULT_DIR, args.subject, args.experiment, args.task + "_GNN.npz")
    print(f"[*] Loading GNN predictions from: {pred_path}")
    
    if not os.path.exists(pred_path):
        raise FileNotFoundError(f"Prediction file not found: {pred_path}. Did run_decoder_GNN.py finish successfully?")
    ...
    print("[*] Generating Null Models for statistical significance...")
    ...
    for reference in args.references:
        ...
        for mname, metric in metrics.items():
            ...
            # Avoid divide by zero if null=1 (for testing)
            if args.null > 1 and story_null_scores.std() > 0:
                window_zscores[(reference, mname)] = (window_scores[(reference, mname)] 
                                                      - window_null_scores.mean(0)) / window_null_scores.std(0)
            else:
                 window_zscores[(reference, mname)] = 0
            ...
            if args.null > 1 and story_null_scores.std() > 0:
                ...
                story_zscores[(reference, mname)] = 0
    
    # [MODIFICATION] Save with _GNN suffix
    ...
    print(f"[*] Evaluation complete. Scores saved to {save_path}.npz")